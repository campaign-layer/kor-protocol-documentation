...
Your app calls the SDK → SDK hits backend → Backend validates API key, encodes transaction, signs it → Returns signature + encoded data

**Step 2: User submission**
SDK routes payload through user's wallet → User confirms → Transaction executes on Base
